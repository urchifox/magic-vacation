...
export default async () => {
  class ResultCanvas extends CanvasManager {
    ...
    drawBlob() {
      ...
      const {topX, topY, tailX, tailY, deltasLength} = this.objects.blob;
      const radius = this.frame.width * (this.objects.blob.radius / 100);
      const centerX = this.frame.width * (topX / 100);
      const centerY = this.frame.height * ((topY + this.objects.blob.radius) / 100);
      this.ctx.beginPath();

      const angle = ((this.objects.blob.angle + 47) * Math.PI) / 180;
      ...
      this.ctx.bezierCurveTo(
          (tailX - this.objects.blob.deltasLength * Math.sin(angle)),
          (tailY + this.objects.blob.deltasLength * Math.cos(angle)),
          centerX + radius,
          centerY + radius,
          centerX,
          centerY + radius,
    ...
    calculateBlob() {
      ...
      this.objects.blob.tailX = centerX + rotatedX;
      this.objects.blob.tailY = centerY + rotatedY;
      this.objects.blob.angle = airplane.rotate;
    }
  }
